...
def annotate_overlap(rows, chunks_by_id):
    ...
    for r in rows:
        ...
        best = best_chunk_match(snippet, cand_chunks) if cand_chunks else None

        # 判定规则
        if best and best["is_substring"]:
            decision, confidence, provisional = "confirmed", "high", False
            rationale = (
                f"normalized_snippet 在 chunk {best['chunk_id']} 中以 substring 精确出现"
            )
        elif best and best["bigram_overlap"] >= OVERLAP_CONFIRM_HI:
            decision, confidence, provisional = "confirmed", "high", False
            rationale = (
                f"bigram 重叠 {best['bigram_overlap']:.2f} >= {OVERLAP_CONFIRM_HI}"
                f"（chunk {best['chunk_id']} 高度相关）"
            )
        elif best and best["bigram_overlap"] >= OVERLAP_CONFIRM_LO:
            decision, confidence, provisional = "confirmed", "low", True
            rationale = (
                f"bigram 重叠 {best['bigram_overlap']:.2f} 在中间区间"
                f"（{OVERLAP_CONFIRM_LO}-{OVERLAP_CONFIRM_HI}），证据不足标记 auto_provisional"
            )
        else:
            decision, confidence, provisional = "reject", "medium", True
            ov = best["bigram_overlap"] if best else 0.0
            rationale = (
                f"bigram 重叠 {ov:.2f} < {OVERLAP_CONFIRM_LO}，候选 chunk 与 "
                f"snippet 文本不一致（候选 {len(cand_chunks)}/{len(cand_ids)} 命中索引）"
            )
        ...
        evidence.append({
            ...
            "matched_chunk_in_corpus": best["chunk_id"] if best else None,
            "bigram_overlap": best["bigram_overlap"] if best else 0.0,
            "is_substring": best["is_substring"] if best else False,
            "review_decision": decision,
            "confidence": confidence,
            "auto_provisional": provisional,
            "rationale": rationale,
            "missing_chunk_ids_in_corpus": missing,
            "annotator": "rule_based_bigram_overlap",
            "model": "none (heuristic)",
            "prompt_hash": None,
            "text_preview": best["text_preview"] if best else None,
        })
...
def annotate_missing(rows, dataset_cases_by_id, by_source_name):
    ...
    for r in rows:
        ...
        ap_matches = []
        all_strong = bool(aps)  # 默认有 aps 且都强匹配才 chunk
        for ap in aps:
            ...
            if best["bigram_overlap"] < MISS_SUBSTR_THRESHOLD and not best["is_substring"]:
                all_strong = False
        ...
        if all_strong:
            ...
            rationale = (
                f"全部 {len(aps)} 个 answer_points 在相关 source 的 chunks 中"
                f"找到强匹配（bigram >= {MISS_SUBSTR_THRESHOLD} 或 substring）"
            )
            # 补标：取每个 answer_point 最佳 chunk 的 source_id + 精确子串作为 snippet
            supplemental_chunks = []
            seen_chunk_ids = set()
            for m in ap_matches:
                if not m.get("best_chunk_id"):
                    continue
                # 取 answer_point(strong match)直接作为 snippet
                # -- match_snippet_to_chunks 规范化后做 substring 判定，
                # 强匹配场景足够触发 exact。
                src_id = next(
        ...
        elif any(m["bigram_overlap"] >= 0.20 for m in ap_matches):
            level, conf, prov = "source", "low", True
            rationale = (
                "部分 answer_points 有较弱 chunk 匹配（" +
                ", ".join(f"{m['bigram_overlap']:.2f}" for m in ap_matches) +
                f"），保守判 source；候选 chunk 在 evidence 中记录"
            )
        else:
...
def main():
    ...
    ov_out, ov_ev, ov_dec, ov_conf = annotate_overlap(overlap_rows, chunks_by_id)
