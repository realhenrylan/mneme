"""稳定 split 生产基线重跑预检（fail-closed）。

校验（任一失败 → 非零退出，明确阻断项）：
1. 锁定配置：load + validate 全字段（dataset/corpus/seed/arms/模型/
   reranker=none/prompt/budgets/arm_selector_policy/split_fingerprint），
   在任何索引/LLM 工作前；
2. 稳定 split：当前 group_aware_split(seed=42) 指纹 == lock.split_fingerprint
   （不依赖 PYTHONHASHSEED）；
3. 双 overlay：dataset_sha256 一致、case refs 有效、annotations 存在于
   dataset；entries 全部被 per-split GT map 消费；双 split truth gate 通过；
4. 索引指纹后验：缓存索引（force_rebuild=False）快照 == lock.index_sha256；
...
LOCK = REBUILD / "lock-production-stable.json"
...
if diffs:
    ...
    notes.append("locked config verified (dataset/corpus/seed/arms/models/"
                 "reranker=none/prompt/budgets/arm_selector_policy)")

# ── 2. 稳定 split 指纹 ───────────────────────────────────────────────
dev, holdout = group_aware_split(cases, seed=42)
split_fp = compute_split_fingerprint(dev, holdout)
fp_diffs = validate_locked_config(
    lock, split_fingerprint=split_fp,
)
if fp_diffs:
