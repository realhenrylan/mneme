...
### Callback example#
 The callback form takes a completion callback function as its last argument and invokes the operation asynchronously. The arguments passed to the completion callback depend on the method, but the first argument is always reserved for an exception. If the operation is completed successfully, then the first argument is
```
null
```
 or
...
### Synchronous example#
 The synchronous APIs block the Node.js event loop and further JavaScript execution until the operation is complete. Exceptions are thrown immediately and can be handled using
```
try…catch
```
...
signal
```
 <AbortSignal> | <undefined> allows aborting an in-progress writeFile. Default:
```
undefined
```
...
```
 values to read a range of bytes from the file instead of the entire file. Both
```
start
```
 and
...
```
 are inclusive and start counting at 0, allowed values are in the [0,
```
...
'close'
```
 event after it has been destroyed. Set the
```
emitClose
```
 option to
...
autoClose
```
 is false, then the file descriptor won't be closed, even if there's an error. It is the application's responsibility to close it and make sure there's no file descriptor leak. If
```
autoClose
```
...
true
```
, the underlying file descriptor is flushed prior to closing it. Default:
```
false
```
.
...
start
```
 option to allow writing data at some position past the beginning of the file, allowed values are in the [0,
```
Number.MAX_SAFE_INTEGER
```
...
offset
```
 <integer> The location in the buffer at which to start filling. Default:
```
0
```
...
position
```
 <integer> | <bigint> | <null> The location where to begin reading data from the file. If
```
null
```
 or
...
-1
```
, data will be read from the current file position, and the position will be updated. If
```
position
```
 is a non-negative integer, the current file position will remain unchanged. Default:
```
null
```
...
buffer
```
 <Buffer> | <TypedArray> | <DataView> A reference to the passed in
```
buffer
```
 argument.
...
buffer
```
 <Buffer> | <TypedArray> | <DataView> A buffer that will be filled with the file data read. Default:
```
Buffer.alloc(16384)
```
...
VersionChanges
 ...
 | v22.17.0  | Marking the API stable.
...
```
 ...
 | v22.15.0  | Removed option to create a 'bytes' stream. Streams are now always 'bytes' streams.
...
autoClose
```
 <boolean> When true, causes the <FileHandle> to be closed when the stream is closed. Default:
```
false
```
...
ReadableStream
```
 will read the file to completion, it will not close the
```
FileHandle
```
...
- Returns: <Promise> Fulfills upon a successful read with the contents of the file. If no encoding is specified (using
```
options.encoding
```
...
readline
```
 interface and stream over the file. See
```
filehandle.createReadStream()
```
 for the options.
...
position
```
 <integer> | <null> The offset from the beginning of the file where the data should be read from. If
```
position
```
 is not a
...
number
```
, the data will be read from the current position. Default:
```
null
```
...
buffers
```
 <Buffer[]> | <TypedArray[]> | <DataView[]> property containing a reference to the
```
buffers
```
 input.
...
bigint
```
 <boolean> Whether the numeric values in the returned <fs.Stats> object should be
```
bigint
```
. Default:
...
len
```
 bytes, it is extended, and the extended part is filled with null bytes (
```
'\0'
```
):
...
offset
```
 <integer> The start position from within
```
buffer
```
...
length
```
 <integer> The number of bytes from
```
buffer
```
 to write. Default:
...
position
```
 <integer> | <null> The offset from the beginning of the file where the data from
```
buffer
```
...
buffer
```
 <Buffer> | <TypedArray> | <DataView> a reference to the
```
buffer
```
 written.
...
filehandle.write()
```
 multiple times on the same file without waiting for the promise to be fulfilled (or rejected). For this scenario, use
```
...
encoding
```
 <string> The expected string encoding. Default:
```
'utf8'
```
...
encoding
```
 <string> | <null> The expected character encoding when
```
data
```
 is a string. Default:
...
```
). Check File access constants for possible values of
```
mode
```
.
 If the accessibility check is successful, the promise is fulfilled with no value. If any of the accessibility checks fail, the promise is rejected with an <Error> object. The following example checks if the file
```
/etc/passwd
```
...
flag
```
 <string> See support of file system
```
flags
```
. Default:
...
path
```
 may be specified as a <FileHandle> that has been opened for appending (using
```
fsPromises.open()
```
).
...
mode
```
 <integer> Optional modifiers that specify the behavior of the copy operation. It is possible to create a mask consisting of the bitwise OR of two or more values (e.g.
```
...
dereference
```
 <boolean> dereference symlinks. Default:
```
false
```
.
...
false
```
, and the destination exists, throw an error. Default:
```
false
```
.
...
false
```
 to ignore it. When ignoring a directory, all of its contents will be skipped as well. Can also return a
```
Promise
```
 that resolves to
...
src
```
 <string> source path to copy.

-
```
dest
```
 <string> destination path to copy to.
...
force
```
 <boolean> overwrite existing file or directory. The copy operation will ignore errors if you set this to false and the destination exists. Use the
```
errorOnExist
```
 option to change this behavior. Default:
```
true
```
.
...
mode
```
 <integer> modifiers for copy operation. Default:
```
0
```
. See
...
recursive
```
 <boolean> copy directories recursively Default:
```
false
```
...
true
```
, path resolution for symlinks will be skipped. Default:
```
false
```
...
cwd
```
 <string> | <URL> current working directory. Default:
```
process.cwd()
```
...
exclude
```
 <Function> | <string[]> Function to filter out files/directories or a list of glob patterns to be excluded. If a function is provided, return
```
true
```
 to exclude the item,
...
undefined
```
. If a string array is provided, each string should be a glob pattern that specifies paths to exclude. Note: Negation patterns (e.g., '!foo.js') are not supported.
...
- Returns: <Promise> Fulfills with the <fs.Stats> object for the given symbolic link
```
path
```
.
...
path
```
 refers to a symbolic link, in which case the link itself is stat-ed, not the file that it refers to. Refer to the POSIX
```
lstat(2)
```
...
mode
```
 <string> | <integer> Not supported on Windows. See File modes for more details. Default:
```
0o777
```
.
...
mode
```
 (permission and sticky bits), or an object with a
```
mode
```
 property and a
...
path
```
 is a directory that exists results in a rejection only when
```
recursive
```
 is false.
...
prefix
```
. Due to platform inconsistencies, avoid trailing
```
X
```
 characters in
...
prefix
```
. Some platforms, notably the BSDs, can return more than six random characters, and replace trailing
```
X
```
 characters in
...
options
```
 argument can be a string specifying an encoding, or an object with an
```
encoding
```
...
fsPromises.mkdtemp()
```
 method will append the six randomly selected characters directly to the
```
prefix
```
...
/tmp
```
, if the intention is to create a temporary directory within
```
/tmp
```
, the
...
flags
```
 <string> | <number> See support of file system
```
flags
```
. Default:
...
mode
```
 <string> | <integer> Sets the file mode (permission and sticky bits) if the file is created. See File modes for more details. Default:
```
0o666
```
...
bufferSize
```
 <number> Number of directory entries that are buffered internally when reading from the directory. Higher values lead to better performance but higher memory usage. Default:
```
32
```
...
Dir
```
 will be an <AsyncIterable> containing all sub files and directories. Default:
```
false
```
...
true
```
, reads the contents of a directory recursively. In recursive mode, it will list all files, sub files, and directories. Default:
```
false
```
.

- Returns: <Promise> Fulfills with an array of the names of the files in the directory excluding
```
'.'
```
 and
...
encoding
```
 property specifying the character encoding to use for the filenames. If the
```
encoding
```
 is set to
...
path
```
 <string> | <Buffer> | <URL> | <FileHandle> filename or
```
FileHandle
```
...
readFile
```
 using an <AbortSignal>. If a request is aborted the promise returned is rejected with an
```
AbortError
```
:
...
encoding
```
 property specifying the character encoding to use for the link path returned. If the
```
encoding
```
 is set to
...
encoding
```
 property specifying the character encoding to use for the path. If the
```
encoding
```
 is set to
...
EPERM
```
 error is encountered, Node.js retries the operation with a linear backoff wait of
```
retryDelay
```
 milliseconds longer on each try. This option represents the number of retries. This option is ignored if the
```
recursive
```
 option is not
...
true
```
, perform a recursive directory removal. In recursive mode, operations are retried on failure. Default:
```
false
```
. Deprecated.
...
retryDelay
```
 <integer> The amount of time in milliseconds to wait between retries. This option is ignored if the
```
recursive
```
 option is not
...
fsPromises.rmdir()
```
 on a file (not a directory) results in the promise being rejected with an
```
ENOENT
```
 error on Windows and an
...
EPERM
```
 error is encountered, Node.js will retry the operation with a linear backoff wait of
```
retryDelay
```
 milliseconds longer on each try. This option represents the number of retries. This option is ignored if the
...
bigint
```
 <boolean> Whether the numeric values in the returned <fs.StatFs> object should be
```
bigint
```
. Default:
...
- Returns: <Promise> Fulfills with the <fs.StatFs> object for the given
```
path
```
.
...
type
```
 argument is not a string, Node.js will autodetect
```
target
```
 type and use
...
path
```
 refers to a symbolic link, then the link is removed without affecting the file or directory to which that link refers. If the
```
path
```
 refers to a file path that is not a symbolic link, the file is deleted. See the POSIX
```
unlink(2)
```
 documentation for more detail.
...
persistent
```
 <boolean> Indicates whether the process should continue to run as long as files are being watched. Default:
```
true
```
.
...
recursive
```
 <boolean> Indicates whether all subdirectories should be watched, or only the current directory. This applies when a directory is specified, and only on supported platforms (See caveats). Default:
```
false
```
.
...
encoding
```
 <string> Specifies the character encoding to be used for the filename passed to the listener. Default:
```
'utf8'
```
.
...
maxQueue
```
 <number> Specifies the number of events to queue between iterations of the <AsyncIterator> returned. Default:
```
2048
```
.
...
'throw'
```
 when there are more events to be queued than
```
maxQueue
```
 allows.
...
'ignore'
```
 means overflow events are dropped and a warning is emitted, while
```
'throw'
```
 means to throw an exception. Default:
```
'ignore'
```
.
...
eventType
```
 <string> The type of change

-
```
filename
```
 <string> | <Buffer> | <null> The name of the file changed.
...
flush
```
 <boolean> If all data is successfully written to the file, and
```
flush
```
 is
...
filehandle.sync()
```
 is used to flush the data. Default:
```
false
```
.
...
fsPromises.writeFile
```
 is a convenience method that performs multiple
```
write
```
 calls internally to write the buffer passed to it. For performance sensitive code consider using
```
...
path
```
 may be specified as a numeric file descriptor that has been opened for appending (using
```
fs.open()
```
 or
...
fd
```
) that is currently in use through any other
```
fs
```
...
mode
```
 is an optional integer that specifies the behavior of the copy operation. It is possible to create a mask consisting of the bitwise OR of two or more values (e.g.
```
fs.constants.COPYFILE_EXCL | fs.constants.COPYFILE_FICLONE
```
).
...
fd
```
 option is supplied, an override for
```
open
```
 is also required. If
...
path
```
 exists by checking with the file system. Then call the
```
callback
```
...
path
```
 exists but points to a non-existent element, the callback will receive the value
```
false
```
.
...
path
```
 <string> | <undefined> Present only if a directory is created with
```
recursive
```
 set to
...
path
```
 is a directory that exists results in an error only when
```
recursive
```
 is false. If
...
recursive
```
 is false and the directory exists, an
```
EEXIST
```
 error occurs.
...
prefix
```
 to create a unique temporary directory. Due to platform inconsistencies, avoid trailing
```
X
```
 characters in
...
mode
```
 sets the file mode (permission and sticky bits), but only if the file was created. On Windows, only the write permission can be manipulated; see
```
fs.chmod()
```
.
...
- Returns: <Promise> Fulfills with a <Blob> upon success.
 ...
 The file must not be modified after the <Blob> is created. Any modifications will cause reading the <Blob> data to fail with a
```
DOMException
```
 error. Synchronous stat operations on the file when the
```
Blob
```
...
position
```
 <integer> | <bigint> | <null> Specifies where to begin reading from in the file. If
```
position
```
 is
...
util.promisify()
```
ed version, it returns a promise for an
```
Object
```
 with
...
fs.read()
```
 method reads data from the file specified by the file descriptor (
```
fd
```
). The
...
length
```
 argument indicates the maximum number of bytes that Node.js will attempt to read from the kernel. However, the actual number of bytes read (
```
bytesRead
```
...
bytesRead
```
 parameter in the callback will indicate the actual number of bytes read, which may be less than the specified
```
length
```
.
...
bytesRead
```
 value to determine how many bytes were actually read from the file. Depending on your application logic, you may need to handle cases where
```
bytesRead
```
...
buffer
```
 <Buffer> | <TypedArray> | <DataView> Default:
```
Buffer.alloc(16384)
```
...
encoding
```
 property specifying the character encoding to use for the filenames passed to the callback. If the
```
encoding
```
 is set to
...
AbortSignal
```
. If a request is aborted the callback is called with an
```
AbortError
```
:
...
fs.readFile()
```
 function buffers the entire file. To minimize memory costs, when possible prefer streaming via
```
fs.createReadStream()
```
.
...
1. If a file descriptor is specified as the
```
path
```
, it will not be closed automatically.

1. The reading will begin at the current position. For example, if the file already had
```
'Hello World'
```
 and six bytes are read with the file descriptor, the call to
```
fs.readFile()
```
...
encoding
```
 property specifying the character encoding to use for the link path passed to the callback. If the
```
encoding
```
 is set to
...
position
```
 is the offset from the beginning of the file from where data should be read. If
```
...
1.  The maximum number of symbolic links is platform-independent and generally (much) higher than what the native
```
realpath(3)
```
...
encoding
```
 property specifying the character encoding to use for the path passed to the callback. If the
```
encoding
```
 is set to
...
true
```
, perform a recursive removal. In recursive mode operations are retried on failure. Default:
```
false
```
.
...
statfs(2)
```
. Returns information about the mounted file system which contains
```
path
```
. The callback gets two arguments
...
listener
```
 <Function> Optional, a listener previously attached using
```
fs.watchFile()
```
...
listener
```
 is specified, only that particular listener is removed. Otherwise, all listeners are removed, effectively stopping watching of
```
filename
```
.
...
options
```
 is provided as a string, it specifies the
```
encoding
```
. Otherwise
...
'change'
```
 event fired by <fs.FSWatcher>, but it is not the same thing as the
```
'change'
```
 value of
...
filename
```
 argument in the callback is only supported on Linux, macOS, Windows, and AIX. Even on supported platforms,
```
filename
```
 is not always guaranteed to be provided. Therefore, don't assume that
```
filename
```
 argument is always provided in the callback, and have some fallback logic if it is
```
null
```
.
...
persistent
```
 that indicates whether the process should continue to run as long as files are being watched. The
```
options
```
...
fs.watchFile()
```
 disappears and reappears, then the contents of
```
previous
```
 in the second callback event (the file's reappearance) will be the same as the contents of
```
previous
```
...
typeof position !== 'number'
```
, the data will be written at the current position. See
```
pwrite(2)
```
.
...
written
```
 specifies how many bytes the passed string required to be written. Bytes written is not necessarily the same as string characters written. See
```
Buffer.byteLength
```
.
...
stdout
```
) a string containing non-ASCII characters will not be rendered properly by default, regardless of the encoding used. It is possible to configure the console to render UTF-8 properly by changing the active codepage with the
```
chcp 65001
```
...
file
```
 is a file descriptor, the behavior is almost identical to directly calling
```
fs.write()
```
 like:
...
fs.write()
```
 is that under some unusual conditions,
```
fs.write()
```
 might write only part of the buffer and need to be retried to write the remaining data, whereas
```
fs.writeFile()
```
...
fs.writeFile()
```
 is called twice in a row, first to write the string
```
'Hello'
```
...
position
```
 is the offset from the beginning of the file where this data should be written. If
```
typeof position !== 'number'
```
...
fs.writev()
```
 multiple times on the same file without waiting for the callback. For this scenario, use
```
fs.createWriteStream()
```
.
...
Error
```
 will be thrown. Otherwise, the method will return
```
undefined
```
.
...
dest
```
 is overwritten if it already exists. Returns
```
undefined
```
...
false
```
 to ignore it. When ignoring a directory, all of its contents will be skipped as well. Default:
```
undefined
```
...
fs.exists()
```
 accepts parameters that are inconsistent with other Node.js callbacks.
```
fs.existsSync()
```
...
undefined
```
, or throws an exception when parameters are incorrect or the operation fails. This is the synchronous version of
```
fs.lutimes()
```
.
...
throwIfNoEntry
```
 <boolean> Whether an exception will be thrown if no file system entry exists, rather than returning
```
undefined
```
. Default:
...
mode
```
 <string> | <integer> Not supported on Windows. Default:
```
0o777
```
.
...
true
```
, the first directory path created. This is the synchronous version of
```
fs.mkdir()
```
.
...
encoding
```
 property specifying the character encoding to use for the filenames returned. If the
```
encoding
```
 is set to
...
encoding
```
 property specifying the character encoding to use for the path returned. If the
```
encoding
```
 is set to
...
undefined
```
. A file descriptor can also be passed as the first argument. In this case,
```
fs.ftruncateSync()
```
 is called.
...
callback
```
 will be called with an <fs.Dirent>, or
```
null
```
 if there are no more directory entries to read.
...
- Type: <string> | <Buffer>
 ...
 The file name that this <fs.Dirent> object refers to. The type of this value is determined by the
```
options.encoding
```
 passed to
...
filename
```
 argument may not be provided depending on operating system support. If
```
filename
```
...
- Returns: <fs.FSWatcher>
 ...
 When called, requests that the Node.js event loop not exit so long as the <fs.FSWatcher> is active. Calling
```
watcher.ref()
```
 multiple times will have no effect.
 By default, all <fs.FSWatcher> objects are "ref'ed", making it normally unnecessary to call
```
watcher.ref()
```
 unless
...
- Returns: <fs.FSWatcher>
 ...
 When called, the active <fs.FSWatcher> object will not require the Node.js event loop to remain active. If there is no other activity keeping the event loop running, the process may exit before the <fs.FSWatcher> object's callback is invoked. Calling
```
watcher.unref()
```
 multiple times will have no effect.
...
- Returns: <fs.StatWatcher>
 ...
 When called, requests that the Node.js event loop not exit so long as the <fs.StatWatcher> is active. Calling
```
watcher.ref()
```
 multiple times will have no effect.
 By default, all <fs.StatWatcher> objects are "ref'ed", making it normally unnecessary to call
```
watcher.ref()
```
 unless
...
- Returns: <fs.StatWatcher>
 ...
 When called, the active <fs.StatWatcher> object will not require the Node.js event loop to remain active. If there is no other activity keeping the event loop running, the process may exit before the <fs.StatWatcher> object's callback is invoked. Calling
```
watcher.unref()
```
 multiple times will have no effect.
...
- Extends: <stream.Readable>
 ...
 Instances of <fs.ReadStream> cannot be constructed directly. They are created and returned using the
```
fs.createReadStream()
```
 function.
...
true
```
 if the underlying file has not been opened yet, i.e. before the
```
'ready'
```
 event is emitted.
...
VersionChanges
 ...
 | v22.0.0  | Public constructor is deprecated.
...
fs.fstat()
```
, and their synchronous counterparts are of this type. If
```
bigint
```
 in the
...
options
```
 passed to those methods is true, the numeric values will be
```
bigint
```
 instead of
...
number
```
, and the object will contain additional nanosecond-precision properties suffixed with
```
Ns
```
.
...
```
 ...
 If the <fs.Stats> object was obtained from calling
```
fs.lstat()
```
 on a symbolic link which resolves to a directory, this method will return
```
false
```
. This is because
...
birthtimeMs
```
 properties are numeric values that hold the corresponding times in milliseconds. Their precision is platform specific. When
```
bigint: true
```
...
birthtimeNs
```
 properties are bigints that hold the corresponding times in nanoseconds. They are only present when
```
bigint: true
```
...
Date
```
 and number values are not connected. Assigning a new number value, or mutating the
```
Date
```
...
atime
```
 "Access Time": Time when file data last accessed. Changed by the
```
mknod(2)
```
,
...
mtime
```
 "Modified Time": Time when file data last modified. Changed by the
```
mknod(2)
```
,
...
ctime
```
 "Change Time": Time when file status was last changed (inode data modification). Changed by the
```
chmod(2)
```
,
...
birthtime
```
 "Birth Time": Time of file creation. Set once when the file is created. On file systems where birthtime is not available, this field may instead hold either the
```
ctime
```
 or
...
mtime
```
 in this case. On Darwin and other FreeBSD variants, also set if the
```
atime
```
...
fs.statfs()
```
 and its synchronous counterpart are of this type. If
```
bigint
```
 in the
...
- Extends <stream.Writable>
 ...
 Instances of <fs.WriteStream> cannot be constructed directly. They are created and returned using the
```
fs.createWriteStream()
```
 function.
...
writeStream
```
. Optionally accepts a callback that will be executed once the
```
writeStream
```
 is closed.
...
F_OK
```
  | Flag indicating that the file is visible to the calling process. This is useful for determining if a file exists, but says nothing about
```
rwx
```
...
X_OK
```
  | Flag indicating that the file can be executed by the calling process. This has no effect on Windows (will behave like
```
fs.constants.F_OK
```
).
...
O_NOATIME
```
  | Flag indicating reading accesses to the file system will no longer result in an update to the
```
atime
```
 information associated with the file. This flag is available on Linux operating systems only.
...
fs
```
 operations accept file paths that may be specified in the form of a string, a <Buffer>, or a <URL> object using the
```
file:
```
 protocol.
...
##### String paths#
 String paths are interpreted as UTF-8 character sequences identifying the absolute or relative filename. Relative paths will be resolved relative to the current working directory as determined by calling
```
process.cwd()
```
.
...
filename
```
 argument may be passed as a <URL> object using the
```
file:
```
 protocol.
...
file:
```
 <URL>s with a host name convert to UNC paths, while
```
file:
```
...
fs.FSWatcher()
```
) use libuv's threadpool. This can have surprising and negative performance implications for some applications. See the
```
UV_THREADPOOL_SIZE
```
...
fsPromises.open()
```
 into a synchronous blocking call. If synchronous operation is desired, something like
```
fs.openSync()
```
 should be used.
...
open(2)
```
; commonly used constants are available from
```
fs.constants
```
. On Windows, flags are translated to their equivalent ones where applicable, e.g.
```
O_WRONLY
```
 to
...
open(2)
```
) causes the operation to return an error if the path already exists. On POSIX, if the path is a symbolic link, using
```
O_EXCL
```
...
```
.
 The behavior of some flags are platform-specific. As such, opening a directory on macOS and Linux with the
```
'a+'
```
 flag, as in the example below, will return an error. In contrast, on Windows and FreeBSD, a file descriptor or a
```
FileHandle
```
 will be returned.
