...
- **答案级评测线立项章程产出（`plans/ANSWER-LEVEL-EVAL-LINE-CHARTER-2026-08-30.md`，纯文档待审批，未进入实施）** — 承接 2.4「source 级通过≠答案正确」教训与 P1.1 分析 M2 移交。①**核心缺口（现状盘点实证）**：`compute_faithfulness` 实为「答案要点术语 vs context ≥50% 命中」的证据覆盖启发式，生成答案文本作为参数传入但不参与判定——「生成答案是否命中 acceptable_answer_points」目前零指标覆盖；generation_runner 为库式无 CLI、无密封产物规范；judge 协议缺失（mixed-030/033 契约错误教训待防御性设计）；无成本治理。②**三方案**：A 纯机械 containment 下界（确定性便宜，同义/翻译型要点结构性漏判）；**B 机械下界 + 受约束 LLM judge（推荐）**——judge 只判机械未命中要点，输出受外置词表 enum {hit,miss,partial} 硬校验 + 证据片段必须为答案文本规范化子串，词表外 fail-closed 重试≤2 次后按 miss 计入并单独披露契约错误率（引擎侧解码约束方向的评测侧预演），成本随质量提升自然收窄；C 最小 refusal 线（不推荐单独立项，作降级退路）。③**里程碑**：M1 CLI+机械指标 TDD+n=10 冒烟 → M2 全量 150 诊断基线（**无门禁**）→ M3 judge 子集+可靠性统计 → M4 预注册阈值门禁另批。④**待批决策点**：Q1 方案（荐 B）、Q2 模型/配额（M2 ≈150 次生成）、Q3 门禁节奏（首轮纯诊断）。

- **评测 runner 切到 v2.1（owner 指示「先不push，做后面的」之一；TDD RED→GREEN；未 push）** — ①**schema 容错读取**（`evaluation/schema.py`，RED 5 failed → GREEN）：`EvalCase.from_dict` 对契约外未知字段一律忽略（v2.x 治理/溯源字段 note/annotation/relevance_level/is_refusal_turn/doc_target 及 relevant_chunks[].chunk_id），`EvalCase` 新增第一类字段 `relevant_chunk_ids`（v2.1 人工终审确认的权威块 ID，与索引 metadata chunk_id 同源同约定；v1 数据集加载后为空列表、检索 runner 沿用 snippet 匹配回退路径，行为不变）；契约姿态保持「annotators write, runners read」，load→save 对扩展字段**不保真**，带扩展字段的发布副本禁止经 EvalCase 回写（docstring 明示）。②**run.py 默认切换**：`--dataset` 默认 v1→v2.1；新增 `DATASET_CORPUS_DEFAULTS` 注册表（v1→test_texts，v2.1→data/v2-corpus/documents/processed），裸名数据集自动解析缺省语料，显式 `--corpus-dir` 永远优先（密封实验自建语料用法零影响）；CLI 冒烟 `--dataset v2.1 --validate-only`：加载 150 例 + validation passed。③**验收**：发布副本 v2.1.jsonl 现可被 `load_dataset` 直接消费（150 例、`validate_dataset` 零警告、en-021 权威块 ID 在加载结果中保留）；定向回归 9 套件 **350 passed / 0 failed**。④**更正记录（诚实性）**：dd66b09 激活产物 manifest 与 CHANGELOG 中「发布副本 schema 投影自检 load_dataset 0 警告」表述易误导——该自检是激活脚本的**内存投影**结论，发布副本本身保留全部扩展字段、在 dd66b09 时点**不能**被 `load_dataset` 直接加载（实测 `RelevantChunk` 拒绝 chunk_id），本次容错读取落地后才可直载；全保真数据集始终在 `revisions/v2.1-human-review-activation/`，本次零数据变更。⑤**边界**：预注册实验仪器（parentchild_ab / multiturn_replay / decompose_ab）默认数据集保持 v1 不动（密封仪器不默默改靶，换靶须预注册修订）；generation_runner 的数据集选择移交答案级评测线立项设计（见同日章程文档）。

- **v2.1 数据集激活：人工终审 overlay 落地为权威真值层，`gate_verdict=ACTIVATED`（owner 指示「启用v2.1（使用子代理）」；子代理执行 + 编排方独立验收；TDD；未 push）** — ①**Phase A 谱系侦察（只读，四问全解）**：草稿 150 行 = case-freeze `partition=new` 全集，legacy_dev 110 行零交集；v2.0.11 冻结候选 136 行全部在池内，差集 14 行为 v2.0.x 链在冻结候选层分批退休者（v2.0.5 zh-033 / v2.0.6 zh-032 / v2.0.8 五条 / v2.0.9 六条 / v2.0.10 multi-019），2026-08-29 终审 150 全 confirmed 按 supersession 取代旧退休；136 交集的 31 处真值差异全部源于 owner 仲裁 8 条证据修复落草稿（f28cce5）；18 条 deferred-owner-decisions 已全消解（batch1 15 归档 + 3 恢复 → batch2 终裁 → overlay 全 confirmed）；rulings 22 条与 6 条 final-rulings case 全在池内；150 case × 5 真值字段草稿==overlay 零不一致。②**Phase B 激活脚本**（`scripts/corpus_v2_v21_activate.py`，TDD 13/13 先测后码）：四门 fail-closed——overlay 150/150 confirmed + SHA-256 独立重算一致 `7d9bceb1…a37e75`、id 双向差集空、真值字段逐位顺序敏感一致、final-rulings case 在池；任一失败零输出；内置双构建字节一致断言。③**产物**（`evaluation/datasets/v2/revisions/v2.1-human-review-activation/`）：`v2.1-dataset.jsonl`（150 行顶层真值字段透传，仅 annotation 三字段如实更新——review_status=`human_review_confirmed_agent_adjudicated`、reviewed_by=`zcode-agent-2026-08-29`、review_notes 记 owner 裁决日期与 v2.1.0，**不伪称真人复核**）；`manifest.json`（六项输入 SHA 快照、门结果、gate_verdict=ACTIVATED、自哈希）；`activation-report.md`（谱系四问、池构成、6 条 supersession 逐 case 链、schema 边界）；发布副本 `evaluation/datasets/v2.1.jsonl` 与数据集逐字节一致（schema 直载能力见同日「评测 runner 切到 v2.1」条目）。④**编排方独立验收**：重跑 activate 四产物 SHA 前后相同（确定性）；150 行 × 13 顶层字段透传零改动 + annotation 契约 150/150 独立全量比对复证；新增测试 13 + apply 冒烟 28 = **41 passed, 0 failed**。⑤**边界发现（如实披露）**：`evaluation/schema.py` `EvalCase.from_dict` 当时不接受草稿扩展字段（note/annotation/relevance_level 等 6 项及 chunk 内 chunk_id），已在同日 runner 切换条目以容错读取解决；mixed-030/mixed-033 旧 retired 裁决由 2026-08-29 授权代理终审 overlay 整体取代（未单独走 owner 仲裁修复，如需补强可 v2.1.x 治理轮专项裁决）。⑥数据卫生：冻结资产（v2.0.x 修订树/v1/sealed）只读零改写；无网络/LLM/API 调用；并行会话文件零触碰；runner 切换与答案级评测线另行处理（runner 已同日完成）。

## 2026-08-29

- **v2 人工终审 Round-2 闭环：owner 仲裁 8 条 → 草稿证据优先修复 → 142 条决策机械转移 + 8 条复核 → apply 150/150 confirmed → `HUMAN_REVIEWED` overlay 生成（verify 复检通过；已 push）** — ①**owner 仲裁批示（逐字）**：「同意六条reject」；noanswer-039「算可答」（机制点名即答）；noanswer-040「实质讲解在就按照讲解回答，然后如实陈述 traits 专章不在」。②**8 条证据优先最小修复**（`v2-cases-draft.jsonl` + `repair-ledger.jsonl` 追加 8 条，既有 10 条不动）：en-044 Rust 侧证据 chunk_37→chunk_43（move 语义）；en-048 问题对齐证据（改问 OVER 语法定义）；en-052 问题改「各自保证」+ Rust 侧补 chunk_53（引用永不悬垂）+chunk_43（杜绝 double-free）；mixed-022 答案点对齐 chunk_5（中英混合条目）；zh-057 章节归属 4.1→3.2（chunk 内目录可证）；noanswer-039/040/050 三条拒答探针翻可答并补证据（chunk_101 平台机制点名 / chunk_46 Copy·Drop 讲解+Chapter 10 指向 / chunk_69+chunk_136 中英双版 10.11 unittest）——band_target 同步 low_answerable，构建元数据保持历史。③**校验链**：修复脚本改动行集断言=目标集 + 全部新 snippet `snippet_is_evidence` 连续性校验（一次围栏代码块未闭合 fail-closed 拦截后修正切点）；`build_pack` 全量校验重建 pack（manifest 五类输入 SHA 更新）；142 条转移断言 pack 级新旧行剔除人工字段逐字节一致（round-1 空白包自 git dba4c81 恢复比对）+ round-1 filled 行一致才照搬；8 条 round-2 复核逐条对照 chunk 原文后 confirmed（审阅人 zcode-agent-2026-08-29，notes 注明依据）。④**apply**：status=overlay、150/0/0、零阻断 → `human-reviewed-truth-overlay.json` + manifest（HUMAN_REVIEWED）；官方 verify：`overlay + manifest chain intact`。⑤**身份声明**：填写/复核均由 AI 代理经 owner 授权执行，overlay reviewer 字段如实记录代理标识；是否据此启用 v2.1 属 owner 另行决策。⑥报告 `agent-fill-report-round2-2026-08-29.md`；数据卫生：chunks/split/sealed 产物零触碰，无 LLM/API 调用路径。
...
- **实施「2.2 附带小项 + 3.1 收尾」（owner 指示「按照这个开工」；TDD 全程；全量回归基线核验）** — ①**A 邻接扩展最小块长守卫**：A1 审计（新脚本 `scripts/audit_chunk_size_distribution.py`，`results/audit-chunk-size/`）证实 v1 736 块中 <20 字符的 12 块**全部为标题/页码残片**（含 run-2 实测 chunk_12="1. 2"），正文短节最小 25 字符；v2 sealed 1006 块 min=22，<20 零命中 → **阈值按审计证据冻结 20**（与初值一致）。TDD 8 用例（短邻居跳过/另一侧仍扩展、select 永不过滤、边界 ±1、`texts=None` 逐字节不变、strip 计长、缺项保守加入）；`src/chunking.py` 加 `MIN_ADJACENT_CHUNK_CHARS` + `texts` keyword-only 参数，`src/rag.py` sync/stream 两处调用点传 `texts=documents`（守卫只过滤邻接候选，select/parent 路径零改动）。②**B parent 划分专项**：B1 审计（`scripts/audit_parentchild_quality.py`，`results/audit-parentchild-quality/`）——tiny child（<30 字符）仅 **3 块 / 609 child（0.5%）**、parent 尺寸健康（min=412/median=1141/max=1982）、child⊆parent 完整性 **201/201 零违规** → **孤例，B2 不实施**（登记已知限制 `B2-DECISION.md`；2/3 已由 A1 守卫拦截，且合并 pass 复杂度对 0.5% 覆盖不成立）。③**run-4 复跑（验收回归，如实付代价）**：n=71 同一 containment-aware 口径，**`STAGE2_22_ACCEPTED` 保持**（mean_delta **+0.1232** ≥0.05、worst **0.0**、142 行密封件自哈希复算 OK；run-3 沙箱索引快照已被环境清理 → 同语料重建索引，dataset_sha256 一致、chunking v3 未动，单变量成立）；**守卫生效机械证明**：run-3 ON 臂 chunk_12 出现 3 次、chunk_26（"2. Background"）2 次 → run-4 均 **0 次**；南京标题块（select 召回）保留（守卫不过滤召回证据）。报告 `results/stage2-parentchild/report-2026-08-29-run4.md`。④**D1 取消机制（TDD 7/7）**：`llm_gateway` 新增 `LLMCancelledError`；`llm_call` keyword-only `cancel_event`——调用前置位零网络零 client（先于配置检查）、attempt 前检查、退避改 `cancel_event.wait(backoff)` 即时唤醒（Timer 0.2s 置位实证 <0.8s 返回）、不可重试；`answer_with_llm_history_stream` 透传 + 流消费逐 chunk 检查（置位 → 关闭响应流 + 抛错，**不 yield API 错误文本**；GeneratorExit/KeyboardInterrupt 后 finally 释放响应流）；`answer_query_stream`/`graph_query_stream`/`LocalRagService.query·graph_query` 全链透传；TUI 生成块捕获 KeyboardInterrupt → 置位 → 「已取消当前回答」→ 回输入提示，**半截答案不入 history（回滚）**——补齐「网络异常不停留 thinking」完成标准。⑤**D2 并发上限可配置（TDD 6/6，subprocess 跨进程验证）**：`RAG_LLM_MAX_CONCURRENCY`（1–32 导入期 fail-fast，错误含配置名），默认 4 行为不变。⑥**D3 错误分类可见（TDD 4/4）**：`get_stats()` 附 `get_call_summary()` 摘要（调用数/错误率/分类分布/token 合计），`render_sidebar` 渲染一行 `LLM Calls 42 (5% err, 1800 tok)`，无调用零噪音。⑦**D5 拆解收益计量（开关 TDD 5/5 + 执行器单测 3/3 + 真实计量）**：`RAG_QUERY_DECOMPOSE`（on 默认/off 导入期 fail-fast）门控 `_plan_query_runtime` decompose 调用（off=单查询直通零 LLM）；`evaluation/decompose_ab.py`——单因子 = decompose 开关（ON 臂冻结 OFF 臂同一 rewrite，模块属性按臂覆盖 finally 恢复）、扩展固定 on、containment-aware recall（run-3 仪器）+ 规划墙钟、沙箱零 trace。**诊断结果**（n=71，无门禁如实报告）：recall OFF 0.5924 → ON 0.5785（**Δ=−0.0139**，改善 2/恶化 3/持平 66，worst −0.5）；规划墙钟 **+577ms**（69→646ms）；子查询分布 OFF 全单查、ON {1:58, 2:10, 3:3}（58/71 被守卫跳过）→ 拆解默认开支大于当前收益，报告 `results/stage3-decompose/report-2026-08-29.md`。⑧**3.1 落账**：现状对齐表 + 完成标准逐条对照写入路线图（模型缓存/timeout/retry/错误分类/token 统计/decompose 守卫均已实现——回填三处声明滞后），D1–D3、D5 补齐真缺口后 **3.1 `[~]→[x]`**（§五进度行同步）。⑨**拟合修复**：`run_chat_loop` 传 `cancel_event` 后，`test_citation_integrity._FakeService.query` 增 `**kwargs` 兼容（25 passed）。⑩全量回归：**v2 语料既有漂移（上轮人工终审 Round-2 未提交修改致 sealed/frozen SHA 不符）占 242 failed + 78 errors + evaluate_v211 5 frags——stash 隔离实验证实与本轮零关系；本轮相关失败仅 citation fake 1 例（已修）；index_lifecycle 1 例为 chromadb compactor 顺序敏感 flaky（单独/重复 5/5 通过）**；排除 v2 漂移类后干净回归 **1473 passed / 1 failed（flaky）/ 1372 deselected**；直接相关套件 156 + 190 + 30 + 25 全绿。
