...
- per-case 指标差异 case 数：**3**；per-case 检索集合差异 case 数：**34**。
- 非零 aggregate Δ：[{'key': 'ndcg@5', 'delta': -0.002155}, {'key': 'ndcg@10', 'delta': -0.002155}, {'key': 'ndcg@20', 'delta': -0.002212}, {'key': 'mrr', 'delta': -0.004823}]

## HNSW 跨构建扰动

- 同索引重复查询逐位一致；跨构建（独立索引）在深 rank 处存在近邻扰动。本阶段确定性复验：
- raw 差异 75 处 / 指标受影响 9 case（明细见 contract-baseline-summary.json determinism）。
